...
class FFMPEGEncoder(object):
    ...
    def on_task_start(self, task, config):
        ...
        if self.ffprobe is None:
            raise DependencyError("ffprobe is required for this module to work. Please install it")
        
        self.ffpmeg = which("ffmpeg")
        if self.ffpmeg is None:
            raise DependencyError("ffmpeg is required for this module to work. Please install it")
        
        self.prepare_config(config)
        
    def on_task_output(self, task, config):
        for entry in task.accepted:
            if (not entry['location']) or (not os.path.exists(entry['location'])):
                ...
                if not out:
                    ...
                    command["commands"].append("-n")
                    if 'temp_dir' in config:
                        temp_dir = config['temp_dir']
                    else:
                        temp_dir = tempfile.gettempdir()
                    tmpfile = os.path.join(temp_dir,filename_without_ext + ".mkv")
                    command["commands"].append(tmpfile)
                    log.info("Encoding %s to %s with command %s" % (filename,tmpfile,string.join(command["commands"], " ")))
                    ts = time.time()
                    ...
                    te = time.time()
                    if return_code == 0:
                        log.info("Encoding of %s succeeded in %s" % (filename,str(datetime.timedelta(seconds=te-ts))))
